...
<html class="fixed">
	<head>
		...
		<title>Rockstar Upload</title>
		<meta name="keywords" content="Rockstar Upload" />
		<meta name="description" content="Rockstar Upload">
					...
					<header class="page-header">
						...
						<div class="right-wrapper pull-right">
							<ol class="breadcrumbs">
								...
								<li><span>Rockstar Upload</span></li>
							</ol>
...
<?php
//เลือกข้อมูล
$sql="SELECT * FROM rockstar_upload ORDER BY id DESC ";
$result=mysql_query($sql)or die(mysql_error());

for($i=0;$row=mysql_fetch_assoc($result);$i++){ 


$sqlc="SELECT * FROM candidate_profile WHERE id = $row[userid] ";
$resultc=mysql_query($sqlc)or die(mysql_error());
...
?>											
											...
											<td><?=$row['id']?></td>
											<td><a href="candidate_profile.php?cus_id=<?=$row['userid']?>" target="_blank"><?=$rowc['name']?></a> <i class="fa fa-hand-o-right "></i> <?=$row['name']?></td>
											...
											<td><?=$row['seniority']?>/ <?=$row['function']?></td>
											<td>
												<?=$row['email']?> / <?=$row['tel']?>
	                            			</td>
...
<div id="modalAnim<?=$row['id']?>" class="zoom-anim-dialog modal-block modal-block-primary mfp-hide">
	<form id="demo-form3" class="form-horizontal mb-lg"  action="process/upload/productDelete.php" method="post" enctype="multipart/form-data">
										<section class="panel">
											<header class="panel-heading">
