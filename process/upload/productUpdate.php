...
if($_FILES['Job_Description']['name']==""){
  ...
  $sql="UPDATE job_post SET title=\"$Title\", department=$Department, nationalities=\"$nationalities\", country=\"$Country\", city=\"$City\", address=\"$Address\", description=\"$Description\", salary=$Min_Salary, max_salary=$Max_Salary, status=$status, status2=$status2, refer_bonus=$Bonus, icon='$randomElement', job_level=$job_leval,  last_update=now(), company=\"$company\" WHERE id=$id";
...
if($_FILES['Job_Description']['name']!=""){
  ...
  //อัพเดทตารางสินค้า
  $sql="UPDATE job_post SET title=\"$Title\", department=$Department, nationalities=\"$nationalities\", country=\"$Country\", city=\"$City\", address=\"$Address\", description=\"$Description\", salary=$Min_Salary, max_salary=$Max_Salary, status=$status, status2=$status2, file_name='$image', refer_bonus=$Bonus, icon='$randomElement', job_level=$job_leval, last_update=now(), company=\"$company\" WHERE id=$id";
  mysql_query($sql)or die(mysql_error());  
