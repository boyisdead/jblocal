...
include('connect.php'); 
//เพิ่มลงฐานข้อมูล
if($_REQUEST['userid']=="" || $_REQUEST['jobid']==""){

 $arr5 = array('status' =>1005, "msg" => "Invalid user id and job id.");
...
$sql="INSERT INTO refer_email(user_id, job_id, links, create_date, refer_email, refer_name, refer_key, refer_phone) VALUES($userid, $jobid, \"$token1\", now(), '$email', '$name', \"$token\", '$phone')";
mysql_query($sql)or die(mysql_error());


$sql="SELECT * FROM job_post WHERE id=$jobid";
...
header("Location: http://www.jbhired.com/".$typer."-".$row['department']);
die();

}else{

header("Location: http://www.jbhired.com/search.php");
die();
...
$sql="INSERT INTO refer_email(user_id, job_id, links, create_date, refer_email, refer_name, refer_key, refer_phone, refer_cv) VALUES($userid, $jobid, \"$token1\", now(), '$email', '$name', \"$token\", '$phone', '$image')";
mysql_query($sql)or die(mysql_error());


if(move_uploaded_file($_FILES['files']['tmp_name'],"../cv/".$image)){
  $error="";
}
//ถ้าอัพโหลดไม่ได้
else {
