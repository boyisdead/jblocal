...
<?php
//เลือกข้อมูล
$sql="SELECT * FROM submit_job ORDER BY id DESC ";
$result=mysql_query($sql)or die(mysql_error());
...
?>											
											...
											<td><?=$row['id']?></td>
											<td><a href="candidate_profile.php?cus_id=<?=$row5['id']?>" target="_blank"><?=$row5['id']?> : <?=$row5['name']?></a></td>


											<td><a href="edit_job.php?id=<?=base64_encode($row['job_id'])?>" target="_blank"><?=$row['job_id']?> / <?=$rowj['title']?></a></td>
...
<span class="label label-success"><?=$status_pro?></span>
													</td>
											...
											<td class="actions" style="font-size:15px;">
												...
												<a href="job-ap-detail.php?id=<?=base64_encode($row['id'])?>" style="font-size: 16px;" class="on-default "><i class="fa fa-cog"></i></a>
...
<div id="modalAnim<?=$row['id']?>" class="zoom-anim-dialog modal-block modal-block-primary mfp-hide">
	<form id="demo-form3" class="form-horizontal mb-lg"  action="process/bank/bankDelete.php" method="post" enctype="multipart/form-data">
										<section class="panel">
											<header class="panel-heading">
