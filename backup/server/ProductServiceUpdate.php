...
//ถ้าไม่แก้ไขรูปภาพ
  ...
  //อัพเดทข้อมูล
  $sql="UPDATE product_aboard SET money_to_m=$money_to_m, endl_date_service='$endl_date_service', detail_for_bear=\"$detail_for_bear\", product=\"$product\", policy_no=\"$policy_no\", first_money=$first_money, company=\"$company\", pa_type=$last_money_to_m, pa_grandtag_name='$pa_grandtag_name', pa_gandtag_pwd='$pa_gandtag_pwd', pa_login_name='$login_name', pa_login_pwd='$login_pwd', pa_policy_date='$date_for_start', pa_pay_date='$pay_date', pa_count_year=$count_year, pa_all_year=$all_year, pa_currency=\"$currency\", pa_amount_per_year=$some_year, pa_money_pay=$money_reverse, pa_how_to_pay=\"$detail_for_recive\", last_money='$last_money', pa_money_reverse=$total_money_reverse, pa_old_resverse=$old_resverse, pa_bank_acc=\"$bank_acc\", pa_name_recive=\"$name_recive\", pa_remark=\"$remark\", last_update=now() WHERE id=$id";
  ...
  echo"<script>window.location='../cus_product_service-$customer_id-$id';</script>";exit;
