...
$sql5="SELECT * FROM product_aboard WHERE member_id= $id AND ((DATE_FORMAT(pa_policy_date,'%Y'))=$my_formaty) ";
	$result5=mysql_query($sql5)or die(mysql_error());
	for($j=0;$row5=mysql_fetch_assoc($result5);$j++){

$startdate22=strtotime($row5[pa_pay_date]);
$mount2 = date("m",$startdate22);
...
$sql4="SELECT * FROM product_aboard WHERE member_id= $id AND ((DATE_FORMAT(pa_policy_date,'%Y'))!=$my_formaty) ";
	$result4=mysql_query($sql4)or die(mysql_error());
	for($i=0;$row4=mysql_fetch_assoc($result4);$i++){

$startdate11=strtotime($row4[pa_pay_date]);
