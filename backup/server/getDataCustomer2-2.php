...
$sql5="SELECT * FROM insurance WHERE member_id= $id AND ((DATE_FORMAT(ins_startDate,'%Y'))=$my_formaty) ";
	$result5=mysql_query($sql5)or die(mysql_error());
	for($j=0;$row5=mysql_fetch_assoc($result5);$j++){

$startdate22=strtotime($row5[ins_deal]);
$mount2 = date("m",$startdate22);

if($mount2=='01'){
$a1 += $row5[ins_moneyPay];
} else if ($mount2=='02'){
$a2 += $row5[ins_moneyPay];
	} else if ($mount2=='03'){
$a3 += $row5[ins_moneyPay];
	} else if ($mount2=='04'){
$a4 += $row5[ins_moneyPay];
	} else if ($mount2=='05'){
$a5 += $row5[ins_moneyPay];
	} else if ($mount2=='06'){
$a6 += $row5[ins_moneyPay];
	} else if ($mount2=='07'){
$a7 += $row5[ins_moneyPay];
	} else if ($mount2=='08'){
$a8 += $row5[ins_moneyPay];
	} else if ($mount2=='09'){
$a9 += $row5[ins_moneyPay];
	} else if ($mount2=='10'){
$a10 += $row5[ins_moneyPay];
	} else if ($mount2=='11'){
$a11 += $row5[ins_moneyPay];
	} else if ($mount2=='12'){
$a12 += $row5[ins_moneyPay];
	} else {
...
$sql4="SELECT * FROM insurance WHERE member_id= $id AND ((DATE_FORMAT(ins_startDate,'%Y'))!=$my_formaty) ";
	$result4=mysql_query($sql4)or die(mysql_error());
	for($i=0;$row4=mysql_fetch_assoc($result4);$i++){

$startdate11=strtotime($row4[ins_deal]);
$mount3 = date("m",$startdate11);

if($mount3=='01'){
$b1 += $row4[ins_moneyPay];
} else if ($mount3=='02'){
$b2 += $row4[ins_moneyPay];
	} else if ($mount3=='03'){
$b3 += $row4[ins_moneyPay];
	} else if ($mount3=='04'){
$b4 += $row4[ins_moneyPay];
	} else if ($mount3=='05'){
$b5 += $row4[ins_moneyPay];
	} else if ($mount3=='06'){
$b6 += $row4[ins_moneyPay];
	} else if ($mount3=='07'){
$b7 += $row4[ins_moneyPay];
	} else if ($mount3=='08'){
$b8 += $row4[ins_moneyPay];
	} else if ($mount3=='09'){
$b9 += $row4[ins_moneyPay];
	} else if ($mount3=='10'){
$b10 += $row4[ins_moneyPay];
	} else if ($mount3=='11'){
$b11 += $row4[ins_moneyPay];
	} else if ($mount3=='12'){
$b12 += $row4[ins_moneyPay];
	} else {
