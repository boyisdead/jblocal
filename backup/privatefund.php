...
<html class="fixed">
	<head>
		...
		<meta name="keywords" content="Private Fund" />
					...
					<!-- start: page -->
						<section class="panel">
							<header class="panel-heading">
								...
								<h2 class="panel-title">Private Fund : รายชื่อลูกค้า </h2>
							</header>
							<div class="panel-body">
								<div class="row">
									<div class="col-sm-6">
										<div class="mb-md">
											<a href="Add_Private_fund" class="btn btn-primary">Add <i class="fa fa-plus"></i></a>
										</div>
									</div>
								</div>
...
//หาจำนวนหน้า
$sql="SELECT COUNT(*) FROM private_fund ";
$result=mysql_query($sql)or die(mysql_error());
$row=mysql_fetch_array($result);
$num=$row['COUNT(*)'];
$numAll=$num;
...
//เลือกข้อมูล
$sql="SELECT * FROM private_fund WHERE member_id = $iduser ORDER BY private_fund.id ASC LIMIT $start_record, $page_size";
$result=mysql_query($sql)or die(mysql_error());
...
switch ($cus_perface) {
 ...
 $private_broker = $row['private_broker'];
...
?>
											...
											<td class="center hide"><?=$row['start_money']?></td>
											<td class="center hide"><?=$row['private_style']?></td>
											<td class="center hide"><?=$row['private_remark']?></td>
											<td class="actions">
												<a href="cus_private_fund-<?=$row['customer_id']?>-<?=$row['id']?>" onclick="clicksound.playclip()" style="font-size: 16px;" class="on-default "><i class="fa fa-play-circle"></i></a>
											...
											<footer class="panel-footer">
												<div class="row">
													<div class="col-md-12 text-right">
														...
														<a class="btn btn-primary " onclick="clicksound.playclip()" href="server/PDelete.php?id=<?=$row['id']?>">Confirm</a>
...
?>
		...
		<!-- Examples -->
		<script src="assets/javascripts/tables/examples.datatables.row.with.details2.js"></script>
