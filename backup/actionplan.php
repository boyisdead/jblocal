...
$sqlcc="SELECT * FROM customer WHERE id = $customer_id";
$resultcc=mysql_query($sqlcc)or die(mysql_error());
$rowcc=mysql_fetch_assoc($resultcc);


$cus_perface = $rowcc['cus_perface'];
					...
					<!-- start: page -->
						<section class="panel">
							<header class="panel-heading">
								...
								<h2 class="panel-title">Action Plan : <?=$cus_perface?><?=$rowcc['cus_name']?> <?=$rowcc['cus_lastname']?> </h2>
							</header>
								...
								<div class="row">
									<div class="col-sm-6">
										<div class="mb-md">
											...
											<a href="Add_actionpaln-<?=$customer_id?>" class="btn btn-primary" onclick="clicksound.playclip()">Add <i class="fa fa-plus"></i></a>
										</div>
								...
								<table class="table table-striped mb-none" id="datatable-editable">
									<thead>
										<tr>
											...
											<th style="width:11%">วันที่งานสำเร็จ</th>
...
//หาจำนวนหน้า
$sql="SELECT COUNT(*) FROM actionplan WHERE customer_id = $customer_id";
$result=mysql_query($sql)or die(mysql_error());
$row=mysql_fetch_array($result);
$num=$row['COUNT(*)'];
$numAll=$num;
...
//เลือกข้อมูล
$sql="SELECT * FROM actionplan WHERE customer_id = $customer_id ORDER BY id DESC LIMIT $start_record, $page_size";
$result=mysql_query($sql)or die(mysql_error());
...
?>
											...
											<td><?=$my_formats2?> <?=$my_formats1?> <?=$my_formats?> </td>
											<td><?=$statuss?> </td>
											<td><?=$row['subport']?></td>
											<td><?=$row['remark']?></td>
											...
											<td class="actions" style="font-size:15px;">
												...
												<a href="detail_actionplan-<?=$row['id']?>-<?=$customer_id?>" style="font-size: 16px;" class="on-default "><i class="fa fa-play-circle"></i></a>
												...
												<a href="print_actionplan-<?=$row['id']?>-<?=$customer_id?>" style="font-size: 16px;" class="on-default "><i class="fa fa-print"></i></a>
...
<div id="modalAnim3<?=$row['id']?>" class="zoom-anim-dialog modal-block modal-block-primary mfp-hide">
										...
										<input name="id" type="hidden" id="id" value="<?=$row['id']?>"/>
												...
												<label for="username" class="col-sm-4 control-label">สิ่งที่ต้องทำ</label>
														<div class="col-sm-7">
												...
												<div class="form-group">
													<label class="col-md-4 control-label" >วันที่งานสำเร็จ :</label>
													<div class="col-md-7">
												...
												<div class="form-group">
									                <label for="detail_action" class="col-sm-4 control-label">หมายเหตุ</label>
									                <div class="col-sm-7">
									                    <textarea class="form-control" name="remark" rows="2"><?=$row['remark']?></textarea>
									                </div>
								            	</div>


								            	<div class="form-group">
									                <label for="statuss" class="col-sm-4 control-label">สถานะงาน</label>
									                <div class="col-sm-7">
...
-->
		...
		<!-- Examples -->
		<script src="assets/javascripts/tables/examples.datatables.editable5.js"></script>
