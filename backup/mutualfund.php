					...
					<!-- start: page -->
						<section class="panel">
							<header class="panel-heading">
								...
								<h2 class="panel-title">Mutual Fund : รายชื่อลูกค้า </h2>
							</header>
							<div class="panel-body">
								<div class="row">
									<div class="col-sm-6">
										<div class="mb-md">
											<a href="Add_Mutual_fund" class="btn btn-primary">Add <i class="fa fa-plus"></i></a>
										</div>
									</div>
								</div>
...
//หาจำนวนหน้า
$sql="SELECT COUNT(*) FROM mutual_fund ";
$result=mysql_query($sql)or die(mysql_error());
$row=mysql_fetch_array($result);
$num=$row['COUNT(*)'];
$numAll=$num;
...
//เลือกข้อมูล
$sql="SELECT * FROM mutual_fund WHERE member_id = $iduser ORDER BY mutual_fund.id ASC LIMIT $start_record, $page_size";
$result=mysql_query($sql)or die(mysql_error());
...
?>	
											...
											<td><?=$row['name']?></td>
											<td><?=$row['phillip_login']?></td>
											<td><?=$mf_saving_plan?></td>
											<td class="center"><?=number_format(($row['mf_goal_ltf']-$row['mf_done_ltf']))?></td>
											<td class="center"><?=number_format(($row['mf_goal_rmf']-$row['mf_done_rtf']))?></td>
											<td class="center"><?=number_format($row['mf_money_bagged'])?></td>

											<td class="center hide"><?=$row['phillip_pwd']?></td>
											<td class="center hide"><?=$row['mf_saving_plan']?></td>
											<td class="center hide"><?=$row['mf_saving_plan_detail']?></td>
											<td class="center hide"><?=$row['mf_open_signature']?></td>
											<td class="actions">
												<a href="cus_mutual_fund-<?=$row['customer_id']?>-<?=$row['id']?>" onclick="clicksound.playclip()" style="font-size: 16px;" class="on-default "><i class="fa fa-play-circle"></i></a>
												<a href="#modalAnim<?=$row['id']?>" class="modal-with-zoom-anim" onclick="clicksound.playclip()" style="font-size: 16px;" ><i class="fa fa-trash-o"></i></a>
											...
											<footer class="panel-footer">
												<div class="row">
													<div class="col-md-12 text-right">
														...
														<a class="btn btn-primary " onclick="clicksound.playclip()" href="server/MDelete.php?id=<?=base64_encode($row['id'])?>">Confirm</a>
