...
switch ($action) {
    case "1":
        if($_REQUEST['startDate1']==""){echo"<script>alert('ยังไม่ใส่วันที่เริ่ม');history.back();</script>";exit;}
		if($_REQUEST['endDate1']==""){echo"<script>alert('ยังไม่ใส่วันที่สิ้นสุด');history.back();</script>";exit;}
			...
			<table class="table table-striped mb-none" id="datatable-editable">
									<thead>
										<tr>
											...
											<th>ชื่อลูกค้า</th>
											<th>ประเภทลูกค้า</th>
											<th>ดิวเบี้ย <?=$startDate?> - <?=$endDate?></th>
...
$sql="SELECT customer.id AS customer_id, customer.cus_perface AS customer_cus_perface, customer.cus_name AS cus_name, customer.cus_lastname AS cus_lastname, customer.cus_types AS cus_type, insurance.ins_deal AS ins_deal FROM customer RIGHT JOIN insurance ON insurance.customer_id=customer.id WHERE (insurance.ins_deal BETWEEN '$startDate' AND '$endDate') AND (customer.member_id= $iduser) ORDER BY insurance.ins_deal DESC ";
...
switch ($cus_cus_type) {
    ...
    default:
        ...
        $cus_perface = $row['customer_cus_perface'];
...
//เลือกข้อมูล
//$sql="SELECT * FROM customer WHERE customer_id = 6 ORDER BY id DESC ";
$sql="SELECT customer.id AS customer_id, customer.cus_perface AS customer_cus_perface, customer.cus_name AS cus_name, customer.cus_lastname AS cus_lastname, customer.cus_types AS cus_type, insurance_al.inal_deal AS ins_deal FROM customer RIGHT JOIN insurance_al ON insurance_al.customer_id=customer.id WHERE (insurance_al.inal_deal BETWEEN '$startDate' AND '$endDate') AND (customer.member_id= $iduser)  ORDER BY insurance_al.inal_deal DESC ";
$result=mysql_query($sql)or die(mysql_error());
for($i=0;$row=mysql_fetch_assoc($result);$i++){ 
...
<!-- //////////////////////////  End  /////////////////////////////////// -->
        ...
        break;
    case "2":
        if($_REQUEST['startDate']==""){echo"<script>alert('ยังไม่ใส่วันที่เริ่ม');history.back();</script>";exit;}
		if($_REQUEST['endDate']==""){echo"<script>alert('ยังไม่ใส่วันที่สิ้นสุด');history.back();</script>";exit;}
	...
	<header class="panel-heading">
		...
		<h2 class="panel-title">ค้นหาตามวันเกิดของลูกค้า</h2>
	</header>
		<div class="panel-body">
			<table class="table table-striped mb-none" id="datatable-editable">
									<thead>
										<tr>
											...
											<th>ชื่อลูกค้า</th>
											<th>ประเภทลูกค้า</th>
											<th>ค้นหาระหว่างวัน <?=$_REQUEST['startDate']?> - <?=$_REQUEST['endDate']?></th>
											<th style="width:10%">Action</th>
										</tr>
									</thead>
									<tbody>
...
$sql="SELECT * FROM customer WHERE DATE_FORMAT(cus_birthday,'%m-%d') BETWEEN '$startDate' AND '$endDate' AND (member_id= $iduser)  ORDER BY id DESC ";
//echo $sql;
$result=mysql_query($sql)or die(mysql_error());
for($i=0;$row=mysql_fetch_assoc($result);$i++){ 
