						...
						<section class="panel">
							...
							<div class="panel-body">
								<div class="row">
									<div class="col-sm-6">
										<div class="mb-md">
											<a href="Add_Product_service" onclick="clicksound.playclip()" class="btn btn-primary">Add <i class="fa fa-plus"></i></a>
										</div>
									</div>
								</div>
...
//หาจำนวนหน้า
$sql="SELECT COUNT(*) FROM product_aboard ";
$result=mysql_query($sql)or die(mysql_error());
$row=mysql_fetch_array($result);
$num=$row['COUNT(*)'];
$numAll=$num;
...
//เลือกข้อมูล
$sql="SELECT * FROM product_aboard WHERE member_id = $iduser ORDER BY product_aboard.id ASC LIMIT $start_record, $page_size";
$result=mysql_query($sql)or die(mysql_error());
...
$time = strtotime($row['pa_policy_date']);
	...
	$sqlP="SELECT * FROM product WHERE service_id = '$row[id]' ORDER BY id ASC LIMIT 1";
	$resultP=mysql_query($sqlP)or die(mysql_error());
	$rowP=mysql_fetch_assoc($resultP);


	$sqlC="SELECT * FROM company WHERE service_id = '$row[id]' ORDER BY id ASC LIMIT 1";
...
?>
											...
											<td><div class="post-date">
			                                    <span class="day"><?=$my_formats2?></span> 
			                                 	 <span class="month"><?=$my_formats1?></span>
			                                    <span class="year"><?=$my_formats?></span>
			                               		 </div>
	                            			</td>
											
											<td class="actions">
												<a href="cus_product_service-<?=$row['customer_id']?>-<?=$row['id']?>" onclick="clicksound.playclip()" style="font-size: 16px;" class="on-default "><i class="fa fa-play-circle"></i></a>
											...
											<footer class="panel-footer">
												<div class="row">
													<div class="col-md-12 text-right">
														...
														<a class="btn btn-primary " onclick="clicksound.playclip()" href="server/ProDelete.php?id=<?=$row['id']?>">Confirm</a>
...
?>
		...
		<!-- Examples -->
		 <script src="assets/javascripts/tables/examples.datatables.editable3-1.js"></script> 
